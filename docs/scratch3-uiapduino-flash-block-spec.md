...
1. 書き込み中に「書き込みの ようす」が割合を返すか。step 4 と 5 の出方が 2 章のとおりか
2. 書き込み後、`D004` として現れて自動で繋ぎ直るか。パレットがブロックに戻るか
3. スケッチ不一致の状態（古い基板）から、説明の中のブロックを押して直せるか。
   **これが本命の経路**
4. まっさらでない基板（同じ `.bin` が既に入っている）で押したとき、
   全セクタが一致して速く終わるか
5. 緑の旗から走らせたとき `SecurityError` で止まるか（ブラウザ版・活性化なしの確認）
6. 書き込みの途中で USB を抜いたとき、`_flashing` が下りて次に押せるか
7. **デスクトップ版で書けるか。** `getDevices()` だけで `B803` が拾えるか
   （活性化の壁が無いことの確認）
8. デスクトップ版で `B803` を要求したときに `D004` を掴まないか。
