...
USB を挿す**手順は残る（README「書き込み方法」の最初の手順）。
...
残り 2 つは、ESM として読むために要る。

**`export default` を足す。** rollup で束ねるので ESM でなければならない。

**⚠ `uint32value_to_array()` の `ret` に `let` を足す。**
元は宣言なしの代入で、暗黙のグローバルになっている。`<script>` から読む限りこれで動くが、
**ES モジュールは常に strict mode** なので `ReferenceError: ret is not defined` で落ちる。

```
[uiapduino] flash failed: ReferenceError: ret is not defined
    at _uint32value_to_array (rv003usbFlasher.js:96:6)
```

**2026-08-14 に実機で踏んだ。** 書き込みモードの基板を選んだ直後、最初の payload を
組み立てるところで落ちる。同じ理由の壊れ方が他に無いことは eslint の `no-undef` で
確かめてある（先頭の `eslint-disable` を外した複製を作って掛ける）。`ret` の 1 か所だけだった。
...
### 済んだこと（2026-08-14 / Xcratch 版 + UIAPduino Pro Micro CH32V003 V1.4）

**ブラウザから実際に書き込めた。** 設計の前提はすべて成立している。

- **パレットのブロックをクリックすれば `requestDevice()` は通る。**
  これが最大の未知数だった。`uiapduinoProcessor.js` の `_connect()` に
  「実機で最初に検証すべき箇所」として残っていた懸念も、これで晴れた
- 書き込みモードでないときは、ブラウザ自身が「対応デバイスが見つかりませんでした」を出す。
  こちらは「基板が書き込みモードになっていません」を出すだけでよい
- 書き込みモードの基板は `32V003` として選択ダイアログに出る
- **「書き込みの ようす」は割合まで含めて動いた。** 最後は「書き込めました」で止まる
- **書き込みの後、自動で繋ぎ直った。** ステータスボタンも接続済みに戻る
- **⚠ 最初の実行は `ReferenceError: ret is not defined` で落ちた。** 5 章の改変 3 を参照
- **⚠ 「UIAPduino への接続が失われました」のアラートが残る。** 下記のとおり、直さないと決めた

**本命の経路も通った。** protocol 5 の基板を繋いで「合いません」の画面を出し、
その説明の中のブロックを押して焼き直し、繋ぎ直ってパレットがブロックへ戻るところまで確認した。

### デスクトップ版でも通った（2026-08-14 / portable 版）

**Xcratch 版と同じように焼けた。** そのうえで、狙いどおりの違いが 2 つ確認できた。

- **デバイス選択のダイアログが出ない。** `setDevicePermissionHandler` が許可しているので
  `getDevices()` だけで `B803` が拾える。**クリックの制限も無い**
- `B803` を要求して `D004` を掴むことは無かった。焼いた後もそのまま繋ぎ直せる

それ以外の挙動は Xcratch 版と同じ。

### 残り

1. まっさらでない基板（同じ `.bin` が既に入っている）で押したとき、
   全セクタが一致して速く終わるか
2. 緑の旗から走らせたとき `SecurityError` で止まるか（ブラウザ版・活性化なしの確認）
3. 書き込みの途中で USB を抜いたとき、`_flashing` が下りて次に押せるか

どれも「壊れていないこと」の確認で、経路そのものは踏み終わっている。

---

## 8. 引き継ぎメモ

### ⚠ 「接続が失われました」のアラートは残る。直さないと決めた（2026-08-14）

繋がっている基板を抜いて書き込みモードに入れると、`PERIPHERAL_CONNECTION_LOST_ERROR` で
アラートが出る。**書き込んで繋ぎ直しても、このアラートだけが取り残される。**

**拡張機能からは閉じられない。**

- 出すのは `scratch-gui/src/lib/vm-listener-hoc.jsx` の
  `vm.on('PERIPHERAL_CONNECTION_LOST_ERROR', onShowExtensionAlert)`
- 閉じるのは **× か 再接続 だけ**。`PERIPHERAL_CONNECTED` を受けて消す分岐は
  `reducers/alerts.js` に無い
- VM 側からアラートを閉じる API は無い

UIAPduino に限らない上流の挙動で、デスクトップ版なら GUI をオーバーレイして直せるが、
**Xcratch 版は xcratch.org の GUI に乗るので手が出せない。** 版によって挙動が変わる方が
筋が悪いので、両方そのままにする。

**本命の経路では出ない。** スケッチが合わない基板は接続前に弾かれ、
`_emitDisconnected()` が false を返すので警告も出ない
（`_handleDisconnectError()` の早期 return）。アラートを踏むのは
「繋がっている基板をわざと抜いたとき」だけ。

**直そうとしないこと。** 同じ道を辿る前にここを読む。

### ⚠ 「新規」でチェックボックスが外れないのも同じ立場（2026-08-14）

「書き込みの ようす」にチェックを入れてステージに出し、**ファイル → 新規**にすると、
**表示は消えるのにチェックだけが残る。** 外して入れ直せば戻る。

**UIAPduino の不具合ではない。** チェックを外す処理は「今あるモニター」だけを回すが
（`scratch-gui/src/containers/blocks.jsx` の `handleMonitorsUpdate`）、
新規で `runtime.dispose()` がモニターの一覧を空にする（`runtime.js:1859`）ため、
**この繰り返しは 1 回も回らない。**

スプライト固有のもの（x座標など）が外れて見えるのは、ブロックの ID に
スプライトの ID が入っていて（`<block id="${targetId}_xposition">`）、
新規で ID が変わり**別のブロックになる**から。外れたのではない。
ID が固定の「タイマー」（`id="timer"`）は同じ症状になる。

**拡張機能のレポーターは必ずこうなる。** ID にスプライトが入らないため。
アラートと同じで、直せるのは GUI の中だけ。Xcratch 版では手が出せないので、
両方そのままにする。
