...
| キーボードのブロック | デバイス側は実装済み。**Scratch 側のブロックは未着手** |
...
| `0x35` | KEY_SHORTCUT | 修飾ビット, キーコード | OK |
...
**キーボードの `0x30` 台はデバイス側だけ先に実装してあります。**
Scratch 側のブロックは後から追加しますが、コマンドを後出しにすると
プロトコルのバージョンをもう一度上げることになり、利用者が基板を 2 回焼き直す
はめになります。そのため受け口だけ先に用意してあります。
...
現在ブロックとして出ているのはマウスだけです。キーボードはデバイス側の実装だけ
先に入れてあり、ブロックは後から追加します。

| ブロック | コマンド |
|---|---|
| `マウスを 右へ [ ] 下へ [ ] 動かす` | `MOUSE_MOVE` |
| `[左] ボタンをクリックする` | `MOUSE_CLICK` |
| `[左] ボタンでダブルクリックする` | `MOUSE_DBLCLICK` |
| `[左] ボタンで 右へ [ ] 下へ [ ] ドラッグする` | `MOUSE_DRAG` |
| `[左] ボタンでドラッグしながら〔 〕`（囲みブロック） | `MOUSE_PRESS` / `MOUSE_RELEASE` |
| `ホイールを [下] に [ ] 回す` | `MOUSE_WHEEL` |
| `キーとマウスをすべて離す` | `PANIC` |
...
### キーボードの制約（ブロック追加前に決めること）

- **日本語は打てません。** HID はキーコードを送るだけで、IME は通りません。
- **US 配列前提です。** 日本語配列の PC では `@ : _ [ ]` などがずれます。
  ブロックに「キーボード配列」の選択を持たせ、Scratch 側で記号のキーコードを
  差し替える必要があります。
- 1 コマンドで送れるのは **30 文字**です（Feature Report 32 バイト − コマンド ID − フラグ）。
  超える分は続きありフラグを立てて連投します。
...
**バージョン 3 では `52 02 01 03` → `52 03 00` になります。**
ダブルクリックとドラッグのブロックとあわせて実機で確認してください。
