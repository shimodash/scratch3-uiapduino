...
uiapruby が生成するファームは 0.5〜2.4ms（duty 6 / 31）を使っていますが、
**その上限を SG-90 に与えるとオーバーシュートして止まらなくなります**（2026-08-08 実測）。

| ブロック値 | duty | パルス幅 | 実測 |
|---|---|---|---|
| 0 | 6 | 0.47ms | 動く |
| 165 | 29 | 2.27ms | ここまでで約 180° 回る |
| 180（旧設定） | 31 | 2.42ms | **オーバーシュートして止まらない** |

**合わせるべきなのは uiapruby ではなく実物のサーボの方です。**
2270µs はこの実測から決めた既定値で、他のサーボなら設定ブロックで変えられます。
